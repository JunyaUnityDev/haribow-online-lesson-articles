---
layout: article
title: "相手の立場で考える、ってどういうことだろう — 人間関係の最初の一歩"
description: "心理学が教える、相手を理解する2つの方法。そして、自分の気持ちにも正直になることの大切さ"
date: 2026-05-01
publish_date: 2026-10-15
category: mindset
slug: perspective-taking-intro
order: 10
---

この記事を読むと、3つのものが手に入る:

1. **「相手の立場で考える」って、本当はどういうことか**
2. **共感と「立場で考える」が、別物だったという科学の発見**
3. **相手を理解する力と、自分に正直になる勇気の、両方を持つ意味**

---

## 💭 プロローグ: フリッキー、また文句を言う

{% include character.html name="flekky" position="left" text="トップさーん、聞いてくださいよ。オレ、最近またチームのメンバーとぶつかってて…" %}

{% include character.html name="top" position="right" text="フッ…またか。今度は何だ?" %}

{% include character.html name="flekky" position="left" text="アイツが、オレの言ってること全然分かってくれないんすよ。何回言っても、噛み合わなくて" %}

{% include character.html name="top" position="right" text="(タバコの煙を吐く)…で、お前は、アイツの言ってることが分かるのか?" %}

{% include character.html name="flekky" position="left" text="えっ?" %}

{% include character.html name="top" position="right" text="お前は、アイツが何を考えてるか、ちゃんと考えたことがあるのか?って聞いてんだ" %}

{% include character.html name="flekky" position="left" text="…(言葉に詰まる)" %}

{% include character.html name="top" position="right" text="フッ。それが答えだろ。いいか若いの。人間関係の話、ちょっと長くなるが、聞いてけ" %}

---

## 🤔 第一章: 「相手の立場で考える」って、よく言われるけど

{% include character.html name="top" position="right" text="『相手の立場に立って考えろ』ってフレーズ、聞いたことあるだろ?" %}

{% include character.html name="flekky" position="left" text="あー、親とか先生とかによく言われるやつっす" %}

{% include character.html name="top" position="right" text="でもよ、これがな、意外と難しい。お前、本当に意味分かってるか?" %}

{% include character.html name="flekky" position="left" text="…相手の気持ちを考える、ってことっすよね?" %}

{% include character.html name="top" position="right" text="それは『共感』って言うんだ。似てるけど、ちょっと違う" %}

{% include character.html name="flekky" position="left" text="えっ、違うんすか!?" %}

{% include character.html name="top" position="right" text="フッ、これが意外と分かってない奴が多い。ここで、世界の研究を引いてもらおう" %}

{% include character.html name="top" position="right" text="マヤ、出番だ" %}

{% include character.html name="maya" position="left" text="(本を持って登場)…はいはい、呼ばれてきました" %}

---

## 🔬 第二章: 科学が分けた「2つの違うもの」

{% include character.html name="maya" position="right" text="2008年、アメリカの心理学者 Adam Galinsky(アダム・ガリンスキー)が、ある研究を発表した。<br><br>場所は **Northwestern University(ノースウェスタン大学)Kellogg School of Management**。論文は **Psychological Science** という、心理学のトップ雑誌に載った" %}

{% include character.html name="flekky" position="left" text="心理学のトップ雑誌…" %}

{% include character.html name="maya" position="right" text="Galinsky たちは、こんな問いを立てた。『人と人が話す時、相手を理解する方法は2種類ある。どっちが効果あるのか?』<br><br>**1つ目: Perspective-taking(立場で考える)** — 頭で『相手はどう考えてるか』を理解する力。<br><br>**2つ目: Empathy(共感)** — 心で『相手の気持ちを感じる』力" %}

{% include character.html name="flekky" position="left" text="…なんとなく似てるけど、確かに違うっすね" %}

{% include character.html name="maya" position="right" text="そう。**頭で考える**か、**心で感じる**か。ここが分かれ道。<br><br>Galinsky たちは、152人の MBA 学生を3つのグループに分けて、交渉ゲームをやらせた。誰が一番、お互いに得する結論にたどり着けるかを調べたの" %}

{% include character.html name="top" position="right" text="(タバコ吸いながら)で、結果は?" %}

{% include character.html name="maya" position="right" text="**立場で考える(perspective-taking)を意識したグループ**が、一番高い割合で、お互いが得する解決策を見つけた。共感だけのグループは、相手を喜ばせようとするあまり、自分が損する選択をしがちだった" %}

{% include character.html name="flekky" position="left" text="えっ、共感って良いことじゃないんすか?" %}

{% include character.html name="maya" position="right" text="共感は良いことよ。でも、**相手の頭の中を理解する**って意味では、立場で考える方が強い、ってことが分かった" %}

---

## 🧠 第三章: 「立場で考える」って、具体的に何をする?

{% include character.html name="flekky" position="left" text="で、立場で考えるって、具体的に何すればいいんすか?" %}

{% include character.html name="maya" position="right" text="3つのステップ。これは Galinsky の論文と、その後の研究で確認されてる方法。<br><br>**ステップ1: 相手の事情を、想像する**<br>その人が今、何を抱えているか。仕事、家族、悩み、疲れ、経験。あんたが見えてないものを、まず想像してみる。<br><br>**ステップ2: 相手の動機を、考える**<br>その人が、なぜそう言ったのか。なぜそう動いたのか。**理由を悪く決めつけない**で、別の理由を3つ考えてみる。<br><br>**ステップ3: 自分が見ている世界が、全部じゃないと知る**<br>あんたが見えてる景色は、その人が見えてる景色とは違う。同じ部屋にいても、見えてるものは別々" %}

{% include character.html name="flekky" position="left" text="…なんか、頭使うっすね" %}

{% include character.html name="top" position="right" text="フッ、当然だ。心で感じるんじゃなくて、**頭で考える**って言っただろ" %}

---

## 🎵 第四章: ダブルダッチで言うと?

{% include character.html name="top" position="right" text="フリッキー、お前のチームの話に戻すぞ。アイツが分かってくれないって言ったよな" %}

{% include character.html name="flekky" position="left" text="うっす…" %}

{% include character.html name="top" position="right" text="アイツは、なぜそう言ったんだ?練習に来ない理由は?" %}

{% include character.html name="flekky" position="left" text="…たぶん、バイト忙しいんすよ。最近シフト増えてるって聞いた" %}

{% include character.html name="top" position="right" text="他には?" %}

{% include character.html name="flekky" position="left" text="…えーと、本当はやる気あるけど、最近上手くできなくて、来づらいのかも…?" %}

{% include character.html name="top" position="right" text="他には?" %}

{% include character.html name="flekky" position="left" text="…家のことで、何かあるのかも、わかんないっすけど" %}

{% include character.html name="maya" position="right" text="ほら、見て。それが**立場で考える**ってこと。あんた、さっきまで『アイツが悪い』って思ってたよね?でも今、3つの可能性が見えた" %}

{% include character.html name="flekky" position="left" text="…マジっすね…" %}

{% include character.html name="top" position="right" text="3つの可能性が見えると、お前の対応が変わる。『なんで来ねえんだよ』じゃなくて、『最近どうした?バイト忙しいか?』って聞ける" %}

{% include character.html name="maya" position="right" text="同じ会話でも、入り口が違うと、結果が変わる。これが研究で証明されてるの" %}

---

## 💬 第五章: 聞くという技術 — Active Listening（能動的傾聴）

{% include character.html name="flekky" position="left" text="で、相手の立場で考える、までは分かったっす。でも、その後どう話しかければいいんすか?" %}

{% include character.html name="maya" position="right" text="次の話。**Active Listening(能動的傾聴)**っていう技術がある" %}

{% include character.html name="flekky" position="left" text="のうどうてき…?" %}

{% include character.html name="maya" position="right" text="提唱したのは、**Carl Rogers(カール・ロジャーズ)**っていう20世紀の有名な心理学者。臨床心理学の世界で広く使われてる方法。<br><br>3つだけ覚えればいい。**繰り返す、確認する、否定しない**" %}

{% include character.html name="flekky" position="left" text="3つだけ?" %}

{% include character.html name="maya" position="right" text="**繰り返す**: 相手が言ったことを、自分の言葉で言い直す。『なるほど、最近忙しいってこと?』みたいに。<br><br>**確認する**: 『そう感じてるってこと?』『こういう意味で合ってる?』と聞く。<br><br>**否定しない**: 相手が話してる間は、否定しない。最後まで聞く。意見が違っても、『そう思ってるんだね』とまず受け取る" %}

{% include character.html name="top" position="right" text="フッ、これがな、できる奴が少ない。みんな、相手が話してる途中から、自分の言いたいこと考え始めちまう" %}

{% include character.html name="flekky" position="left" text="…オレも、それやってるかも…" %}

{% include character.html name="top" position="right" text="お前だけじゃない、みんなだ。だが、これができる奴は、自然と人が集まってくる。NYで学んだ、本物の話だ" %}

---

## 🌱 第六章: なぜ、これで人間関係が変わるのか

{% include character.html name="flekky" position="left" text="マヤさん、なんで立場で考える + 傾聴 で、人間関係が変わるんすか?" %}

{% include character.html name="maya" position="right" text="科学的に分かってることが、いくつかある。<br><br>**1つ目: バイアスが減る**<br>2011年の Todd, Bodenhausen, Richeson, Galinsky の研究(Journal of Personality and Social Psychology)で、**立場で考える練習をした人は、無意識のバイアスが減った**ことが分かった" %}

{% include character.html name="flekky" position="left" text="バイアス?" %}

{% include character.html name="maya" position="right" text="無意識の決めつけ、思い込み。『この人はこういう人だ』って先入観が、薄まる。<br><br>**2つ目: 協力行動が増える**<br>同じ研究グループ(Galinsky & Moskowitz、2000年)で、**立場で考える人は、見知らぬ人にも協力的になりやすい**ことが分かった。<br><br>**3つ目: 信頼が生まれる**<br>2008年の Galinsky の研究で、立場で考える行動は、**お互いの信頼を深める**ことが確認されてる" %}

{% include character.html name="flekky" position="left" text="バイアス減る、協力増える、信頼が生まれる…全部、人間関係に大事なやつっすね" %}

{% include character.html name="top" position="right" text="(深く頷く)…俺がNYで覚えたことを、今の若いのが研究で証明してる。時代だな" %}

---

## 💪 エピローグ: でも、自分の気持ちにも正直に

{% include character.html name="flekky" position="left" text="トップさん、マヤさん。オレ、明日の練習でアイツに話しかけてみるっす" %}

{% include character.html name="top" position="right" text="どう話しかける?" %}

{% include character.html name="flekky" position="left" text="『なんで来ないんだよ』じゃなくて、『最近どう?バイト忙しい?』って聞いてみるっす。あと、アイツが話してる途中で、口出さない" %}

{% include character.html name="maya" position="right" text="(笑って)悪くないじゃん、その3秒で得た学び" %}

{% include character.html name="top" position="right" text="フッ、お前、ちょっと大人になったな" %}

{% include character.html name="flekky" position="left" text="うっす!" %}

---

{% include character.html name="flekky" position="left" text="っていうかマヤさん、ちょっと聞きたいんすけど" %}

{% include character.html name="maya" position="right" text="ん?" %}

{% include character.html name="flekky" position="left" text="マヤさん、いつもこんな冷静に相手の立場で考えてるんすか?なんか、すげー人格者って感じがするっす" %}

{% include character.html name="maya" position="right" text="(本を閉じる)…あー。それね" %}

{% include character.html name="top" position="right" text="(タバコの煙を吐く)…マヤ、お前、何かあるな?" %}

{% include character.html name="maya" position="right" text="あたしね、昔チームにいたのよ。窪塚洋介みたいな喋り方の一般人が" %}

{% include character.html name="flekky" position="left" text="…?" %}

{% include character.html name="maya" position="right" text="『俺ぇ〜、マジでぇ〜、それマジリスペクトっすわぁ〜』みたいな" %}

{% include character.html name="flekky" position="left" text="あ、あの感じ…" %}

{% include character.html name="maya" position="right" text="あたし、それ、本当に無理だったの。価値観とかじゃなくて、生理的に。3ヶ月我慢して、相手の立場で考えようとしてみた。事情を想像した。動機を考えた" %}

{% include character.html name="top" position="right" text="…で?" %}

{% include character.html name="maya" position="right" text="ぶちのめした" %}

{% include character.html name="flekky" position="left" text="えっ" %}

{% include character.html name="top" position="right" text="フッ" %}

{% include character.html name="maya" position="right" text="言葉でね。論理的に。1時間。本人と他のメンバーが見てる前で。完全勝利" %}

{% include character.html name="flekky" position="left" text="お、おう…" %}

{% include character.html name="maya" position="right" text="あんたたち、勘違いしないで。今日の話は本当よ。研究も全部正しい。相手の立場で考えるのは、人生で大切なこと。<br><br>でもね、もう一つ大切なことがある。<br><br>**自分の気持ちにも、正直になること**" %}

{% include character.html name="maya" position="right" text="どんなに相手の立場を考えても、どうしても無理なものは無理。我慢し続けたら、自分が壊れる。その時は、ちゃんとぶつかる。相手の立場を考えた上で、自分の本音を伝える。それも、人生に必要なことなの" %}

{% include character.html name="flekky" position="left" text="(小声で)…おお…" %}

{% include character.html name="top" position="right" text="(小声で)…おお…" %}

{% include character.html name="maya" position="right" text="何よ、その目は" %}

{% include character.html name="flekky" position="left" text="いや、なんでも、ないっす…" %}

{% include character.html name="top" position="right" text="(タバコ消しながら)…俺は何も見てない、何も聞いてない" %}

{% include character.html name="maya" position="right" text="(笑って)Whatever. じゃ、今日の話、覚えとけ。3つよ" %}

---

**人間関係に悩んだ時。**

『相手が悪い』と決めつける前に、3つだけ。

1. 相手の立場で考える（頭で、相手の事情を想像する）
2. 共感する（心で、相手の気持ちに寄り添う）
3. 能動的に聞く（繰り返す、確認する、否定しない）

世界中の研究で確認されてる、人間関係の最初の一歩。

そして、もう一つ。

**自分の気持ちにも、正直に。**

両方を、忘れないこと。

---

## 👥 登場人物

### 🧢 フリッキー(主人公)

- 19歳、神奈川出身、大学生 + コンビニ夜勤バイト
- ダブルダッチ歴1年8ヶ月
- カッコつけてるけど、根は純粋
- 小学生時代、テストで全部の問題が解けなかった時に何も書かずに出して「間違ってない=ノーミス=100点です」と先生に言い張った事がある
- 口癖: 「マジで?」「やばっ」「すげー」「うっす」
- 今回: チームメンバーとの行き違いで、トップに相談に来る

### 😠 トップ(ベテラン)

- 52歳、大阪出身、関内のレコード屋店主
- 元 NYC のヒップホップ・ダンサー、ダブルダッチ歴30年以上
- Grandmaster Flash と一晩飲んだ事がある
- ある日、みんなの飲み会でタクシー代をケチるために「俺歩きで帰るわ〜」と宣言、終電2分前まで粘って奇跡のタクシー代ゼロを狙った
- 「フッ…」が口癖、不器用な優しさ
- 今回: NY時代に学んだ「人付き合いの本質」を、フリッキーに伝える

### 👩 マヤ(知性派の姉さん)

- 30歳、横浜出身、ダブルダッチパフォーマー兼カルチャー誌ライター
- 文化人類学修士、ダブルダッチ歴15年、英仏語堪能
- マイナーなアーティスト好きと言って、有名になった瞬間「もう違うんだよね」と言い出す。本人は「商業化されると本質が…」と論理的に説明できる
- 観葉植物の世話が下手で全部枯らす、窪塚洋介のファンだが、彼みたいな話し方をする一般人は嫌い
- 口癖: 「で、それのソースは?」「あんたねぇ…」「Whatever.」「悪くないじゃん」(最大の褒め言葉)
- 今回: ゲスト出演として、研究と科学的根拠を補強する役

---

## 📚 参考・出典

### 主要な研究

- **Galinsky, A. D., Maddux, W. W., Gilin, D., & White, J. B. (2008)** — "Why It Pays to Get Inside the Head of Your Opponent: The Differential Effects of Perspective Taking and Empathy in Negotiations." *Psychological Science, 19(4), 378-384.* [PubMed](https://pubmed.ncbi.nlm.nih.gov/18399891/)
- **Galinsky, A. D., & Moskowitz, G. B. (2000)** — "Perspective-Taking: Decreasing Stereotype Expression, Stereotype Accessibility, and In-Group Favoritism." *Journal of Personality and Social Psychology, 78(4), 708-724.*
- **Todd, A. R., Bodenhausen, G. V., Richeson, J. A., & Galinsky, A. D. (2011)** — "Perspective Taking Combats Automatic Expressions of Racial Bias." *Journal of Personality and Social Psychology, 100(6), 1027-1042.*
- **Carl Rogers (1957, 1980)** — Active Listening / Person-Centered Therapy の提唱者。"On Becoming a Person" (1961), "A Way of Being" (1980)

### 「自分の気持ちに正直になる」の心理学的裏付け

- **Deci, E. L., & Ryan, R. M. (1985)** — 自己決定理論（Self-Determination Theory）。自分の本音と一致した行動が、心理的幸福感に直結することを実証。"Intrinsic Motivation and Self-Determination in Human Behavior" (Plenum, 1985)
- **Wood, A. M., Linley, P. A., Maltby, J., Baliousis, M., & Joseph, S. (2008)** — 本来性（authenticity）研究。自分らしくあることと心の健康の相関。"The authentic personality: A theoretical and empirical conceptualization and the development of the Authenticity Scale" (*Journal of Counseling Psychology*, 55(3), 385-399)

### 補足リソース

- [Harvard Program on Negotiation — Perspective Taking and Empathy in Business Negotiations](https://www.pon.harvard.edu/daily/dealmaking-daily/to-better-understand-other-negotiators-consider-the-context-nb/)
- [Association for Psychological Science — It Pays to Know Your Opponent](https://www.psychologicalscience.org/news/releases/it-pays-to-know-your-opponent-success-in-negotiations-improved-by-perspective-taking-but-limited-by-empathy.html)

---

<small>※ この記事で紹介した研究は、それぞれの論文・著作の文脈で発表されたものです。本文中の解釈や具体例（ダブルダッチへの置き換え等）は、原典の主旨を保ちつつ HARIBOW 受講者向けに編集しています。</small>
