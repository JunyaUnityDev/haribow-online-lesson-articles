---
layout: article
title: "2人の刑事が、女の子たちのために縄を作った話 — ダブルダッチの起源 前編 —"
description: "1971年、ある刑事が公園で見たもの。街角の遊びが、Lincoln Center に立つまでの物語"
date: 2026-05-01
publish_date: 2026-06-15
category: culture
slug: dd-origin-part1
order: 6
---

この記事(前編)を読むと、3つのものが手に入る:

1. **ダブルダッチが「文化」になった瞬間の物語**
2. **1971年、ある刑事が公園で見たもの**
3. **街角の遊びが、Lincoln Center に立つまで**

(続きは7月15日、後編へ)

---

## 💭 プロローグ

{% include character.html name="flekky" position="left" text="マヤさん、オレ、ダブルダッチって言葉も知らんかったっす。ヒップホップなら、5月にトップさんが教えてくれたから、最近覚えたけど" %}

{% include character.html name="maya" position="right" text="(本を閉じて)悪くないじゃん、その正直さ。じゃ、教えてあげる。あんたが今跳んでる縄の話を" %}

{% include character.html name="flekky" position="left" text="お願いします!オランダから来たんですよね、あれ?" %}

{% include character.html name="maya" position="right" text="(微笑んで)それも、ある。でも、それだけじゃないの。あたしが面白いと思うのは、別の話" %}

---

## 🌊 第一章:始まりは、女の子たちの街角だった

{% include character.html name="maya" position="right" text="時代は1940〜50年代。場所はニューヨーク、ハーレム" %}

{% include character.html name="maya" position="right" text="黒人の女の子たちが、家の物干しロープを2本、街角に持ち出した" %}

![ニューヨークの街角でダブルダッチを跳ぶ少女たち]({{ '/assets/images/harlem-girls-jump-rope.jpg' | relative_url }})
<small>写真出典: <a href="https://medium.com/africana-feminisms/double-dutch-a-classic-for-whom-ee47acf3531c" target="_blank" rel="noopener">Medium — Africana Feminisms「Double Dutch: A Classic For Whom?」</a></small>

{% include character.html name="maya" position="right" text="友達が回す、自分が跳ぶ、誰かが歌う。『Miss Mary Mack(ミス・メアリー・マック)』っていう、当時流行ってた歌のチャント" %}

{% include character.html name="flekky" position="left" text="歌いながら跳ぶんすか?" %}

{% include character.html name="maya" position="right" text="そう。『Miss Mary Mack, Mack, Mack, all dressed in black, black, black...』みたいな、口で言葉のリズムを刻みながら、足でロープのリズムも刻む" %}

{% include character.html name="maya" position="right" text="大人たちの目を盗んで、自分たちだけの空間を作ってた。それは『遊び』じゃない。あの子たちにとっての『居場所』だった" %}

{% include character.html name="flekky" position="left" text="居場所…" %}

{% include character.html name="maya" position="right" text="あの時代、男の子はバスケ、フットボール、ベースボール。女の子のスポーツは、何もなかった。何も。だから女の子たちは、縄2本で、自分の世界を作った" %}

{% include character.html name="maya" position="right" text="NDDL(National Double Dutch League)の現会長、ローレン・ウォーカーさんはこう言ってる。『Double Dutch is Black girl magic.(ダブルダッチは、黒人少女の魔法)』" %}

{% include character.html name="flekky" position="left" text="魔法…" %}

{% include character.html name="maya" position="right" text="あんた、ちゃんと聞いてるじゃん。悪くない" %}

---

## 👮 第二章:1971年、Marcus Garvey Park の自転車レース

{% include character.html name="maya" position="right" text="1971年、ハーレムのマーカス・ガーヴィ公園" %}

![ハーレム、Marcus Garvey Park（マーカス・ガーヴィ公園）の階段]({{ '/assets/images/marcus-garvey-park-harlem.jpg' | relative_url }})
<small>撮影: Linda Fletcher（2022年）／ <a href="https://commons.wikimedia.org/wiki/File:Marcus_Garvey_Park,_Harlem,_NYC.jpg" target="_blank" rel="noopener">Wikimedia Commons</a> ／ <a href="https://creativecommons.org/licenses/by-sa/4.0/" target="_blank" rel="noopener">CC BY-SA 4.0</a></small>

{% include character.html name="maya" position="right" text="NYPDの刑事に、ユリシーズ・ウィリアムズって男がいた。みんなは『マイク・ザ・コップ(街のマイク)』って呼んでた" %}

{% include character.html name="flekky" position="left" text="黒人警官っすか?" %}

{% include character.html name="maya" position="right" text="そう。コミュニティ・アフェアーズ部門っていう、街の人たちと向き合う部署にいた。子供たちと顔見知り。街のことなら、誰よりも知ってた" %}

{% include character.html name="maya" position="right" text="ある日、マイクは自分たちで企画した自転車レースを開催した。子供向け、1マイル(約1.6km)の短いレース" %}

{% include character.html name="maya" position="right" text="男子の部:大盛り上がり。女子の部:全員が惨敗だった" %}

{% include character.html name="flekky" position="left" text="えっ、惨敗?" %}

{% include character.html name="maya" position="right" text="あとからマイク本人がインタビューでこう言ってる。『My God the girls did terrible!(ああ、女の子たちは惨敗だった)』って。<br><br>レースが終わって、街を歩いて帰る道。マイクは独り言をつぶやいてた。『この街には、女の子たちが輝ける場所がない』って。<br><br>そして、ふと足が止まる。<br><br>街角で、女の子たちが縄を回してる。跳んで、笑って、自分たちのリズムで" %}

{% include character.html name="flekky" position="left" text="!" %}

{% include character.html name="maya" position="right" text="マイクは思った。『これだ』って" %}

---

## 📞 第三章:深夜の電話、2人の刑事

{% include character.html name="maya" position="right" text="マイクが電話したのは、同僚の刑事、デイビッド・A・ウォーカー。みんなはデイブって呼んでた" %}

{% include character.html name="maya" position="right" text="デイブも同じNYPDコミュニティ・アフェアーズ。ハーレム32分署所属" %}

{% include character.html name="flekky" position="left" text="同じ部署か" %}

{% include character.html name="maya" position="right" text="そう。2人はパートナー、捜査もコミュニティ活動も一緒にやる仲だった" %}

{% include character.html name="maya" position="right" text="その夜、デイブは家にいた。妻のジュディスと、生まれたばかりの娘ローレンと。あ、このローレンが、さっき言った今のNDDL会長" %}

{% include character.html name="flekky" position="left" text="えっ、デイブの娘さん?" %}

{% include character.html name="maya" position="right" text="そう。父の意志を継いで、今もダブルダッチを世界に広めてる。<br><br>電話に出るデイブ。マイクは興奮気味に話す。<br><br>『Dave、聞いてくれ。あの女の子たちの遊び — Double Dutch、俺たちの手で、sport にできる』<br><br>デイブの返事はこうだったって、後年マイクはこう振り返ってる。『From that point on, David and I began to contrive the sport of double dutch.(そこから俺とデイブで、ダブルダッチを sport として組み立て始めた)』" %}

{% include character.html name="flekky" position="left" text="すげー、警察官2人で…" %}

{% include character.html name="maya" position="right" text="デイブはね、Lauren が赤ん坊の時に、街の子供たち向けの自転車安全プログラムも始めてた。Skyscraper Cycling Classic っていう、いま全米でも有名な自転車レース。あれ、デイブが作ったの" %}

{% include character.html name="maya" position="right" text="『コミュニティに何かを返す』が、デイブの人生のテーマだった。妻のジュディスは結婚前からそれを知ってて、こう言ってる。『私は、社会に何かを返すことを使命とする男と結婚するんだ、って分かってた』って" %}

{% include character.html name="flekky" position="left" text="奥さんも理解してたんすね…" %}

{% include character.html name="maya" position="right" text="2人の警官、1人の妻、1人の赤ん坊。物語が始まった夜は、そういう夜だったの" %}

![David A. Walker（NDDL創設者、ハーレムの街角にて）]({{ '/assets/images/david-walker-portrait.jpg' | relative_url }})
<small>写真出典: <a href="http://www.nationaldoubledutchleague.com/founder/" target="_blank" rel="noopener">National Double Dutch League — Founder David A. Walker</a></small>

![Ulysses "Mike the Cop" Williams（NYPD制服姿）]({{ '/assets/images/ulysses-williams-portrait.jpg' | relative_url }})
<small>写真出典: <a href="https://x.com/JunHaratake/status/943363832042229760/photo/2" target="_blank" rel="noopener">@JunHaratake on X</a></small>

---

## 🎯 第四章:他のスポーツから「いいとこ取り」したルール

{% include character.html name="maya" position="right" text="電話のあと、マイクとデイブは毎晩集まって議論した。<br><br>『ただ跳ぶだけじゃ、評価できない。客観的なルールが要る』<br><br>2人は、他のスポーツの本を山積みにして、いいとこ取りを始めた。<br><br>陸上から:スピードテスト。『2分間で何回跳べるか』<br>体操から:コンパルソリー(規定演技)。『右足ターン2回、左足ターン2回、右が上のクロス2回、左が上のクロス2回、ハイステップ10回(膝を腰の高さまで)』<br>フィギュアスケートから:フリースタイル。3分間の自由演技" %}

{% include character.html name="flekky" position="left" text="全部、他から持ってきた…" %}

{% include character.html name="maya" position="right" text="そう。『ゼロから新しいものを作る』んじゃなくて、『すでにあるものを組み合わせて、新しい意味を持たせる』。これが2人のやり方だった" %}

{% include character.html name="maya" position="right" text="こうして、ハーレムの2人の刑事の手で、『誰がすごいか』を客観的に決められる仕組みが完成した" %}

{% include character.html name="flekky" position="left" text="街の遊びが、競技に変わる瞬間っすね" %}

{% include character.html name="maya" position="right" text="悪くない理解の仕方じゃん" %}

---

## 🎉 第五章:1974年2月14日、バレンタインデーの奇跡

{% include character.html name="maya" position="right" text="1974年2月14日。バレンタインデー。<br><br>場所はハーレムのIntermediate School 10、West 149th Street。今は Frederick Douglas Academy っていう学校。<br><br>初の公式ダブルダッチ・トーナメントが開催された。<br><br>参加者は、5年生から8年生まで。約600人の子供たち" %}

{% include character.html name="flekky" position="left" text="600人!?" %}

{% include character.html name="maya" position="right" text="スポンサーはRoyal Crown Cola(ロイヤル・クラウン・コーラ)。当時の人気飲料メーカー。<br><br>ニューヨークで一番権威ある雑誌の一つ、The New Yorker が、3月11日号でこの大会の記事を書いた。<br><br>街角の女の子たちの遊びが、初めて『公式』になった日。<br><br>その会場で、デイブとマイクは、警官の制服のまま、子供たちの輪の中にいた。<br><br>想像してみて。冬の体育館。蛍光灯の下、子供たちの笑い声、縄が床を打つ音、子供たちの靴底のキュッキュッていう音" %}

![1974年バレンタインデー、Intermediate School 10 体育館での大会のイメージ]({{ '/assets/images/ai-generated/dd-origin-part1-ch5-1974-tournament.jpeg' | relative_url }})
<small>※ 写真はイメージです。AI（Gemini）で生成。</small>

{% include character.html name="maya" position="right" text="2人の刑事は、その音の中に立ってた" %}

{% include character.html name="maya" position="right" text="同じ年、彼らは American Double Dutch League(ADDL)を設立。デイブが会長に就任" %}

{% include character.html name="maya" position="right" text="そして大会の舞台は、すぐにLincoln Center(リンカーン・センター)に移った" %}

{% include character.html name="flekky" position="left" text="リンカーン・センター、聞いたことあるかも…" %}

{% include character.html name="maya" position="right" text="メトロポリタン・オペラがある、世界最高峰の文化施設。クラシックの聖地。そこで、ハーレムの女の子たちが縄を跳んだ" %}

![Lincoln Center for the Performing Arts プラザ — メトロポリタン・オペラ建物と Revson Fountain]({{ '/assets/images/lincoln-center-overview.jpg' | relative_url }})
<small>撮影: Ajay Suresh（2019年）／ <a href="https://commons.wikimedia.org/wiki/File:Lincoln_Center_Overview_(48047495362).jpg" target="_blank" rel="noopener">Wikimedia Commons</a> ／ <a href="https://creativecommons.org/licenses/by/2.0/" target="_blank" rel="noopener">CC BY 2.0</a></small>

{% include character.html name="maya" position="right" text="物干しロープから始まった遊びが、世界の文化施設に立つまで、たった3年" %}

---

## 💭 エピローグ(前編)

{% include character.html name="flekky" position="left" text="マヤさん、続き!続き気になる!" %}

{% include character.html name="maya" position="right" text="Whatever. 来月また来な" %}

{% include character.html name="maya" position="right" text="Lincoln Center で跳んだ女の子たちが、次に何を起こしたか — 来月、教えてあげる" %}

{% include character.html name="flekky" position="left" text="えーー!マジで気になりすぎだろ!ヒップホップとどう繋がるんすか!?" %}

{% include character.html name="maya" position="right" text="(本を閉じて、微笑む)…それが、続きを待つってことよ" %}

{% include character.html name="maya" position="right" text="(独り言)あんたみたいな子に話せて、悪くない夜だったわ" %}

---

## 👥 登場キャラクター紹介

{% include character.html name="flekky" position="left" text="オレ、フリッキー!19歳、ダブルダッチ歴1年8ヶ月の大学2年生っす。今回は質問担当!" %}

**フリッキー(主人公)**

- 19歳、神奈川出身、大学生 + コンビニ夜勤バイト
- カッコつけてるけど、根は純粋
- 小学生時代、テストで全部の問題が解けなかった時に何も書かずに出して「間違ってない=ノーミス=100点です」と先生に言い張った事がある
- 口癖:「マジで?」「やばっ」「すげーすげー」

{% include character.html name="maya" position="right" text="あたし、マヤ。30歳、ダブルダッチパフォーマー兼カルチャー誌ライター。文化人類学修士。今回はあんたたちに、縄の歴史を語る役" %}

**マヤ(姉さん)**

- 30歳、横浜出身、ダブルダッチパフォーマー兼カルチャー誌ライター
- 文化人類学修士、ダブルダッチ歴15年、英仏語堪能
- マイナーなアーティスト好きと言って、有名になった瞬間「もう違うんだよね」と言い出す。本人は「商業化されると本質が…」と論理的に説明できる
- 観葉植物の世話が下手で全部枯らす、窪塚洋介のファンだが、彼みたいな話し方をする一般人は嫌い
- 口癖:「で、それのソースは?」「あんたねぇ…」「Whatever.」「悪くないじゃん」(最大の褒め言葉)

---

## 📚 参考・出典

### Double Dutch の歴史

- [National Double Dutch League — History](https://nationaldoubledutchleague.com/history/)
- [Wikipedia — Double Dutch (jump rope)](https://en.wikipedia.org/wiki/Double_Dutch_(jump_rope))
- [Vice — Double Dutch's Forgotten Hip-Hop Origins](https://www.vice.com/en/article/double-dutchs-forgotten-hip-hop-origins-456/)
- [NPR — How the Fantastic Four took Double Dutch to new heights](https://www.npr.org/2022/12/04/1140504217/double-dutch-fantastic-four-holiday-classic)
- [Kennedy Center — Double Dutch: Jump In!](https://www.kennedy-center.org/education/resources-for-educators/classroom-resources/media-and-interactives/media/hip-hop/double-dutch-jump-in/)

### David A. Walker / Ulysses Williams

- [NDDL — Founder David A. Walker](http://www.nationaldoubledutchleague.com/founder/)
- [Ebony — The Coolest Black Family in America: The Walkers](https://www.ebony.com/coolest-black-family-america-walker/)

### 音源・映像(参考)

- The Incredible Bongo Band - Apache(1973):前編で言及した「ヒップホップの国歌」
- Frankie Smith - "Double Dutch Bus"(1981)
- Pick Up Your Feet: The Double Dutch Show(1981、エミー賞受賞ドキュメンタリー)

---

<small>※ ダブルダッチの起源には諸説あります。本記事(前編)は、最も広く伝えられている流れ — ハーレムの黒人少女たちの街角遊びと、1970年代のNYPD刑事 David A. Walker / Ulysses Williams による sport 化 — を中心に構成しています。</small>
